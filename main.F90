...
!             given in many papers as f(sigma) fit.
!             See README for available options and input parameters.
...
write(20,*) '#    sigma       f(sigma)     M [Msun/h]   dn/dlogM [(Mpc/h)^-3]'
write(20,*) '# --------------------------------------------------------------'
do i=1, nstep
...
M  = M_min / 10.0d0**(dM)
do i = 1, nstep+2
   s2 = sigma2((3.0d0*M/(4.0d0*pi*rho_0))**(1.0d0/3.0d0))
   f(i,1) = M
