!---------------------------------------------------------------------
! Module calculates growth factor for a given redshift
!
! use as: call linear_growth(z, growth, tol)

! where:
!         z is the redshift (cosmology is taken from CommonData)
!         growth is the result
!         tol is the (optional, default 1.0d-6) tolerance

!     The code calcualtes D(a)/a by integration of the differential
!     equations for the evolution of perturbations (Peebles, sec. 10).


module ModuleGrowthFactor
...
subroutine linear_growth(z, delta, tol)
implicit none
double precision, intent(in) :: z
double precision, optional, intent(in) :: tol
double precision, intent(out) :: delta
integer, parameter :: n=2
double precision, dimension(n) :: y, yprime
double precision :: eps, a, a_in, ddot, norm
! declarations for dverk
integer :: ind
double precision :: c(24), w(n,9)
...
y(1) = a_in
y(2) = 0.0d0
call dverk(n, fcn, a_in, y, 1.0d0, eps, ind, c, n, w)
norm = y(1)
...
y(2) = 0.0d0
a = 1.0d0/(1.0d0+z)
call dverk(n, fcn, a_in, y, a, eps, ind, c, n, w)
call fcn(n, a, y, yprime)
delta = y(1)/norm
ddot  = y(2)/norm
end subroutine linear_growth


subroutine fcn(n, a, y, yprime)
!  The system of equations, see Peebles eq. 10.3
!  yprime(1) is ddelta/da, yprime(2)=d^2d
!  y(1) is delta(t) and y(2) is ddelta/dt.
use ModuleCommonData, ONLY: omega_0, h, w_0, w_a
implicit none
integer, intent(in) :: n
double precision, intent(in) :: a, y(n)
double precision, intent(out) :: yprime(n)
double precision :: H0, Hz
H0 = h*100.d0/3.08567758d19  ! mpc in km
Hz = H0*sqrt(omega_0/(a**3) + (1.0d0-omega_0)*a**(-3.d0*(1.d0+w_0+w_a) * &
                                                  exp(-3.0d0*w_a*(1.0d0-a))))
yprime(1) = y(2) / (a*Hz)
yprime(2) = -2.d0*(y(2)/a) + 1.5d0*omega_0*H0*H0*y(1)/(Hz*a**4)
end subroutine fcn
