...
! use as: s2 = sigma2(R)      where R is the spherical top-hat
!                                   radius in Mpc/h
!         mf = tinker(sigma)  where sigma is the variance of
...
double precision, save :: omhh, f_nu, alpha_nu, sound_horizon, beta_c
double precision :: f_bar, f_nub, f_c, f_cb, p_c, p_cb
double precision :: obhh, z_drag, z_equality, y_d, R_drag, R_equality, k_equality
...
double precision function s2_int(k)
use ModuleCommonData, ONLY: R_th, AA, n, tf, pi
implicit none
double precision, intent(in) :: k
double precision, parameter :: inv2pi = 1.d0/(2.d0*pi*pi)
double precision :: kR, window_f, trf
...
kR = k*R_th
window_f = 3.d0*((sin(kR) - kR*cos(kR))/(kR**3))
s2_int   = inv2pi * k*k * AA*k**n * trf*trf * window_f*window_f
end function s2_int


double precision function s2_inv(k)
implicit none
double precision, intent(in) :: k
if (k .le. tiny(1.0d0)) then
   s2_inv = 0.0d0
else
   s2_inv = 1.d0/(k*k) * s2_int(1.d0/k)
endif
end function s2_inv


double precision function sigma2(R)
use ModuleIntegrator, ONLY: integrate
use ModuleCommonData, ONLY: R_th
implicit none
double precision, intent(in) :: R
R_th = R  ! save in CommonData
sigma2 = integrate(s2_int, 0.d0, 10.d0) + integrate(s2_inv, 0.d0, 0.1d0)
end function sigma2
